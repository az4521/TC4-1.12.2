...
public class InventoryFake implements IInventory {
   ...
   public InventoryFake(ItemStack[] stackList) {
      this.stackList = new ItemStack[stackList.length];
      Arrays.fill(this.stackList, ItemStack.EMPTY);
      for(int i = 0; i < stackList.length; ++i) {
         this.stackList[i] = stackList[i] == null ? ItemStack.EMPTY : stackList[i];
      }
   }

   public InventoryFake(ArrayList stackList) {
      this((ItemStack[])stackList.toArray(new ItemStack[0]));
   }
